...
class Login(Ui_login, QMainWindow):
    def __init__(self):
        ...
        #self.login_window.username_lineEdit.setText('ssast2021')
        #self.login_window.password_lineEdit.setText('%SSAST!Fall42')
        self.login_window.ip_lineEdit.setText('192.168.44.133')
        #self.login_window.ip_lineEdit.setText('166.111.80.66')
...
class FTPClient(QMainWindow):
    ...
    def Login(self):
        try:
            ...
            if self.login.login_window.ip_lineEdit.text() != '192.168.44.133':
                self.client.vitural_machine = False
                self.client.localIP = get_local_IP(self.client.vitural_machine)
        ...
        user_cmd = "USER " + self.login.login_window.username_lineEdit.text()
    ...
    def port_mode(self):
        ...
        self.window.cmd_window.radioButton_port.setChecked(True)
    ...
    def upload_local(self, src_filename, dest_filename):
        ...
        if self.client.mode == 'PORT':
            if self.client.vitural_machine:
                self.client.port()
            else:
                self.client.pasv()
    ...
    def download_remote(self, src_filename, dest_filename):
        if self.client.mode == 'PASV' or not self.client.vitural_machine:
            print(self.client.pasv())
        if self.client.mode == 'PORT' and self.client.vitural_machine:
