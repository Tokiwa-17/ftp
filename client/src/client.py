...
def get_local_IP(vitural_machine=True):
    try:
        ...
        s.close()
    #if not vitural_machine:
    #    import requests
    #    ip = print(requests.get('http://ifconfig.me/ip', timeout=5).text.strip())
    return ip
...
class Client():
    def __init__(self, ftp):
        ...
        self.vitural_machine = True
        self.localIP = get_local_IP(self.vitural_machine)
    ...
    def pasv(self):
        ...
        if self.data_address[-1] == ')':
            self.data_address = self.data_address[:-1]
    ...
    def retr(self, src_path, dest_path, filesize, offset, progress_bar):
        ...
        if self.mode == 'PASV' or not self.vitural_machine:
            self.data_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            try:
        ...
        elif self.mode == 'PORT' and self.vitural_machine:
            try:
